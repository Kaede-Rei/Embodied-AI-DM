...
# 4. 终端输入 ls -l /dev/com-* 查看当前创建的符号链接及其对应的实际设备路径
#    示例输出：
#    /dev/com-1.4-tty -> ttyACM0
#    /dev/com-2.1-tty -> ttyUSB0
#    /dev/com-3.2-video -> video2
# 5. 在 LeRobot 配置中使用这些固定路径，例如：
#    FOLLOWER_PORT="/dev/com-1.4-tty"
#    LEADER_PORT="/dev/com-2.1-tty"
#    CAMERAS_CONFIG='{"context": {"type": "opencv", "index_or_path": "/dev/com-3.2-video", ...}}'
#
...
# 显示当前连接的设备
for dev in /dev/com-*; do
    [ -e "$dev" ] && echo "-   $dev -> $(readlink $dev)"
done

echo "- "
echo "- 使用固定路径更新 LeRobot 配置，示例："
echo '-   FOLLOWER_PORT="/dev/com-1.4-tty"'
echo '-   LEADER_PORT="/dev/com-2.1-tty"'
echo '-   CAMERAS_CONFIG='"'"'{"context": {"type": "opencv", "index_or_path": "/dev/com-3.2-video", ...}}'"'"
echo "=========================================="