...
# 3. 训练数据集 $USER/name 已存在并可访问
#
# 使用方法示例：
# ./train_dk1.sh --policy_repo_id $USER/act_dk1_model
# ./train_dk1.sh --policy_repo_id $USER/act_dk1_more_data --steps 30000 --batch_size 4
# ./train_dk1.sh --policy_repo_id $USER/act_dk1_model --push_to_hub --steps 50000
#
# 支持的参数：
# --policy_repo_id <repo_id> 必须：Hugging Face 模型仓库 ID（如 $USER/act_dk1_model），用于标识版本并自动生成 output_dir
# --dataset_repo_id <repo_id> 数据集 repo_id（默认 $USER/dk1_test，可覆盖）
...
# 默认参数配置
DATASET_REPO_ID="$USER/dk1_test" # 默认数据集 repo_id
...
if [[ -z "$POLICY_REPO_ID" ]]; then
  echo "错误：必须指定 --policy_repo_id（例如 $USER/act_dk1_model）"
  exit 1
fi

# 从 policy_repo_id 提取仓库名称部分（例如 $USER/act_dk1_model → act_dk1_model）
...
echo "- 数据集 Repo ID: $DATASET_REPO_ID"
...
# 将 PID 写入文件（便于后续查找）
echo $TRAIN_PID > "train_${REPO_NAME}.pid"

# 训练启动后的提示信息
echo "训练已在后台启动（进程 PID: $TRAIN_PID）"
echo "日志输出至 train_${REPO_NAME}.log"
echo "PID 已保存至 train_${REPO_NAME}.pid"
